...
class RListService: ObservableObject {
    ...
    func addPinToQuickList(_ place: PinData, context: NSManagedObjectContext, userId: String) -> Bool {
        let quickList = getOrCreateQuickList(in: context, userId: userId)
        return addPlaceToList(place, list: quickList, context: context)
    }
    
    /// Removes a pin from the Quick List
    func removePinFromQuickList(_ place: PinData, context: NSManagedObjectContext, userId: String) -> Bool {
        let quickList = getOrCreateQuickList(in: context, userId: userId)
        return removePlaceFromList(place, list: quickList, context: context)
    }
    
    /// Toggles a pin in the Quick List
    func togglePinInQuickList(_ place: PinData, context: NSManagedObjectContext, userId: String) -> Bool {
    ...
    private func createPinDataFromAsset(_ asset: PHAsset, context: NSManagedObjectContext) -> PinData {
    ...
    private func addPlaceToList(_ place: PinData, list: RListData, context: NSManagedObjectContext) -> Bool {
        // Check if already exists
